...
description: 'Get the position from an event like: - `touchstart` - `touchmove` - `touchend` - `touchcancel` - `mousedown` - `mouseup` - `mousemove` - `mouseover` - `mouseout` - `mouseenter` - `mouseleave` - `pointerdown` - `pointerup` - `pointermove` - `pointerover` - `pointerout` - `pointerenter` - `pointerleave`'
type: 'Function'
status: 'stable'
since: '1.0.0'
...
<span class="docmap_name">event<span class="docmap_required">*</span></span><span class="docmap_default">-</span> <span class="docmap_type">MouseEvent&vert;PointerEvent&vert;TouchEvent</span>

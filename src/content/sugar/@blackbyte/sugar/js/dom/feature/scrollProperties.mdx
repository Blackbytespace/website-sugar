...
description: 'This feature allows you to automatically apply css properties on the body element depending on the scroll position. The properties that are applied are: - `--scroll-x`: The current scroll x position - `--scroll-y`: The current scroll y position'
type: 'Function'
status: 'stable'
since: '1.0.0'

...
description: 'Check if the passed HTMLElement is visible or not. Visible mean that it has not an opacity of 0, not a visibility of hidden and not a display of none and `return !!(e.offsetWidth || e.offsetHeight || e.getClientRects().length);`'
type: 'Function'
status: 'stable'
since: '1.0.0'
...
<p class="docmap_description">If the element is visible or not</p>
</li>
</ol>
</div>
